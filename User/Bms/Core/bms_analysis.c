...
static void BMS_AnalysisTaskEntry(void *paramter)
{
	...
	{			
        ...
        UpdateStoredEnergy(); // 调用更新存储能量的函数
...
float CalculateStoredEnergy() {
    // 这里添加具体的能量计算逻辑
    // 例如：根据电池电压、电流和时间来计算能量
    float voltage = BMS_MonitorData.BatteryVoltage;
    float current = BMS_MonitorData.BatteryCurrent;
    float time = 1.0; // 假设时间为 1 小时
    return voltage * current * time;
}

// 更新存储能量的值
void UpdateStoredEnergy() {
    BMS_AnalysisData.StoredEnergy = CalculateStoredEnergy();
}
