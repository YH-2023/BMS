...
BMS_EnergyDataTypedef BMS_EnergyData = 
{
    ...
    .EnergyRecoveryStartVoltage = ENERGY_RECOVERY_START_VOLTAGE,
	.EnergyRecoveryStopVoltage = ENERGY_RECOVERY_STOP_VOLTAGE,
};
...
static void BMS_BalanceTimerEntry(void *paramter)
{
    ...
    BMS_HalCtrlEnergyRecovery(BMS_STATE_DISABLE); // 停止能量回收模块
	...
	if (BMS_ProtectAlert == FlAG_ALERT_NO)
	{
		...
		switch(BMS_GlobalParam.SysMode)
		{
			...
			case BMS_MODE_DISCHARGE:
			{
                ...
                // 放电过程中检查是否满足能量回收条件
                else if (BMS_MonitorData.BatteryVoltage >= BMS_EnergyData.EnergyRecoveryStartVoltage) //>=4000mv
                {
                    BMS_HalCtrlEnergyRecovery(BMS_STATE_ENABLE);
                }
                else if (BMS_MonitorData.BatteryVoltage < BMS_EnergyData.EnergyRecoveryStopVoltage) //<3800mv
                {
                    BMS_HalCtrlEnergyRecovery(BMS_STATE_DISABLE);
                }
			}break;
        ...
        // 检查是否需要停止能量回收
		if (BMS_MonitorData.BatteryVoltage < ENERGY_RECOVERY_STOP_VOLTAGE)
		{
			BMS_HalCtrlEnergyRecovery(BMS_STATE_DISABLE);
		}
	}
