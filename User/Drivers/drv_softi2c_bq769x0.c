...
// 控制能量回收装置
void BQ769X0_ControlEnergyRecovery(BQ769X0_EnergyRecoveryControlTypedef ControlType, BQ769X0_StateTypedef NewState)
{
	if (NewState == BQ_STATE_ENABLE)
	{
		// 开启能量回收装置的硬件接口
		HAL_GPIO_WritePin(ENERGY_RECOVERY_GPIO_PORT, ENERGY_RECOVERY_GPIO_PIN, GPIO_PIN_SET);
	}
	else
	{
		// 停止能量回收装置的硬件接口
		HAL_GPIO_WritePin(ENERGY_RECOVERY_GPIO_PORT, ENERGY_RECOVERY_GPIO_PIN, GPIO_PIN_RESET);
	}
}
